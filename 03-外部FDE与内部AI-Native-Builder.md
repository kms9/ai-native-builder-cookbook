...
AI Native Builder 不是 FDE 的新名称，也不是 FDE 进入企业后的下一阶段。它描述的是另一份工作合同：有人站在企业内部的能力建设立场，把一次 AI 项目从问题发现推进到生产使用，并帮助团队或公司持续走向 AI Native。
...
> **AI Native Builder 是企业内部推动团队或公司走向 AI Native 的建设者。他从真实业务结果出发，发现哪些问题能用、值得用 AI 解决，把 AI 的效果变成稳定、可评估、可治理的系统能力，并把 AI 应用推进生产环境，让真实团队持续使用。**

这一定义可以压缩成三条课程承诺：

1. 发现业务中哪些问题能用、值得用 AI 解决；
2. 把 AI 的效果稳定下来；
3. 把 AI 应用推进生产环境，让它在真实工作中可用。

这一定义没有要求一个人独自完成所有专业工作。Builder 可以是单一岗位，也可以由一个内部小组共同承担。三条承诺会展开为七段相互连接的责任：

```text
发现 → 重构 → 构建 → 评估 → 治理 → 采用 → 复用
```

七个动词不是 FDE 能力清单的改写，也不是用来背诵的流程标签。发现与重构帮助团队选择值得建设的问题；构建、评估和治理帮助团队稳定 AI 效果；采用与复用检查应用能否进入真实工作，并逐渐成为组织能力。几段责任会互相往返，不是三条彼此隔离的流水线。
...
FDE 与 AI Native Builder 都会进入真实工作、缩小问题、构建可运行切片、观察生产采用。这些相似动作来自企业 AI 落地面对的共同困难，不表示一个角色继承自另一个角色。

外部 FDE 的首要目标，是让供应方的模型或平台在客户现场产生结果，并把重复模式和失败反馈给供应方产品。内部 AI Native Builder 的首要目标，是推动本公司的团队和流程走向 AI Native；他持有本组织的业务优先级、知识、权限、评估、采用和长期能力建设。
