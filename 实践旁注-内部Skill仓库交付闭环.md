---
title: "实践旁注｜内部 Skill 仓库交付闭环"
desc: "将真实 Skill 仓库实践分别映射到第 5—10 章，用于理解需求分流、工作流、状态、评估、采用和资产化。"
aliases:
  - "第5至10章实践旁注"
  - "Skill仓库案例章节映射"
tags:
...
# 实践旁注｜内部 Skill 仓库交付闭环

本文件是 [真实实践｜从多维表需求到 MR 与上线](案例A-从多维表需求到MR与上线.md) 的章节化阅读入口。

正文继续使用“澄川工业”演示完整方法；本旁注保留真实实践中的不规则性：业务人员在多维表提交需求，编排平台调用 Skill 分析并修改代码，维护者审核 MR，CI 和测试环境负责确定性验证。简单需求可以快速交付，复杂需求则因缺少业务建模持续被拒绝。
...
相关正文：[第 5 章｜从工具愿望到可验证场景](05-从工具愿望到可验证场景.md)
...
如果删除产品名后无法说明任务，当前还没有通过 G1；如果对象、关系和验收仍然含糊，不应进入编码。

---

## 对应第 6 章：业务人员不需要迁移到 Agent 平台

相关正文：[第 6 章｜重构人—AI—系统工作流](06-重构人AI系统工作流.md)
...
相关正文：[第 7 章｜把业务变成可运行系统](07-把业务变成可运行系统.md)
...
## 对应第 8 章：Agent 自省不能替代独立质量门禁

相关正文：[第 8 章｜用评估与治理证明系统可控](08-用评估与治理证明系统可控.md)
...
| 门禁 | 主要问题 | 证据 |
|---|---|---|
| 需求门禁 | 做的是被确认的需求吗？ | 目标行为、非目标、验收示例 |
| 代码门禁 | 实现是否符合架构与兼容？ | MR diff、静态检查、Reviewer 意见 |
| 测试门禁 | 正常、边界和回归是否通过？ | CI、单测、集成测试、失败记录 |
| 业务门禁 | 最终行为是否可用且符合责任？ | 提交人或业务 Owner 验收 |
...
## 对应第 9 章：运行一个月不等于形成采用

相关正文：[第 9 章｜从试点到真实采用](09-从试点到真实采用.md)

### 现场现象

流程已经运行一个多月，有人能够提交需求，部分需求能够上线。这些事实证明流程不是静态 Demo，但还不能回答：
...
相关正文：[第 10 章｜把一次项目变成组织能力](10-把一次项目变成组织能力.md)
...
配套填写材料见：[练习册扩展｜需求分流与自动交付](练习册扩展-需求分流与自动交付.md)。
