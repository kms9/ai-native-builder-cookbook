...
原项目作者加入会议，边解释边改配置。系统最终在沙盒跑了起来，但第二团队不知道哪些改动属于本地适配，哪些已经破坏原有安全约束。文件全部复制成功，能力没有被复制。
...
试点结束后的默认动作常常是“做成产品”。这个词掩盖了几种投入完全不同的选择。

一次性脚本解决一次明确问题，由执行者保留必要说明后结束。团队产品服务一组持续用户，需要入口、状态、权限、反馈、支持和版本。共享组件为多个项目提供稳定接口，但不直接承诺完整业务结果。平台则要长期服务多个团队，并承担兼容、容量、治理和迁移责任。
...
产品化会产生持续承诺。有人要处理 CRM 变化、资料更新、模型版本、用户反馈、故障、权限和成本。若项目只能依赖原作者临时帮助，试点通过也不代表组织有能力经营它。
...
长期投入还要比较替代方案。若一个表单改动就能解决负责人缺失，不应把它塞进 AI 产品；若某类会议每季度只发生一次，保留人工流程可能更便宜。产品化判断要允许部分能力回到普通软件或人工制度。

这个判断不同于第 9 章的试点决定。上一章回答当前证据支持怎样继续；本章还要决定由谁长期承担、如何复用和何时退役。

## 产品交付包要让服务能够被经营

共享目录最初按文件类型组织：提示词、代码、流程图、评估表、会议记录。第二团队知道里面有什么，却不知道怎样完成一项任务。项目组改成围绕运行责任组织交付。

产品概要先限定用户、任务、允许字段和禁止范围。使用说明从会议结束写到 CRM 最终状态，保留正常、阻塞和未知状态的处理。系统说明连接上下文来源、`CRMChangeSet`、工具、权限和轨迹，不再单独展示一张架构图。
...
这次结果证明共享核心可以被第二团队安装、运行和解释，也证明渠道业务场景尚未准备好。项目组分别记录“技术资产复用通过”和“场景价值待验证”，没有用一个复用成功标签代替两种结论。
...
[NIST AI RMF Core](https://airc.nist.gov/airmf-resources/airmf/5-sec-core/)把持续监控、角色责任和安全退役都放在治理结果中。对 AI Native Builder 而言，能关停一个不再可靠的系统，与能启动它同样重要。
...
华南渠道场景继续停在影子验证。两条政策任务正确阻塞，说明系统守住了边界，也说明该场景尚不能产生完整结果。缺少批准来源之前，不增加工具权限。

平台化延期。现在只有一个第二团队测试，业务语义差异明显，长期维护成本也未得到稳定证据。团队记录平台候选问题，但不先建设一个“通用销售 AI 平台”。

这个决定保留了已经验证的共性，也拒绝把局部成功无限外推。组织能力的标志不是资产数量，而是下一个团队能更少依赖作者，安全地完成适用性判断、构建、评估和停止。

## 几种会制造“伪复用”的做法

把文件上传共享盘，只解决了存放问题。没有适用条件、依赖、版本、负责人和失败例，第二团队仍要重新调查。

让原作者全程迁移，会把专家服务算成资产能力。复用测试要限制帮助，记录介入时间和原因。

过早平台化会冻结尚未稳定的错误抽象。一个团队出现需求是项目，多个团队独立出现同一机制才构成更强的共性信号，仍需比较泛化和维护成本。

只保存成功案例，会让下一个团队重复事故。失败条件、检测方式、修复和回归任务应进入资产。

资产没有停止状态也很危险。过期资料比没有资料更容易误导，因为使用者会把它当作组织批准的答案。
