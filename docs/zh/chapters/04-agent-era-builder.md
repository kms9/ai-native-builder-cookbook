...
| AI 对一个动作的权限 | 人的责任 | 澄川示例 |
|---|---|---|
| 只提供建议 | 人理解后自行决定并执行 | 建议下一步 |
| 准备可检查的待提交内容 | 人确认后提交 | 准备 CRM 更新预览 |
| 在低风险、明确范围内执行 | 人抽查并处理异常 | 更新个人内部备注 |
| 按已批准条件连续执行多步动作 | 人监督指标与高风险例外 | 在特定客户类型内创建内部任务 |
| 在较开放范围内规划和行动 | 人主要管理政策和异常 | 澄川当前不采用 |

让 AI 准备内容、由人确认后执行，不一定比让 AI 连续执行多步动作差。若动作不可逆、涉及对外承诺或错误难以及时发现，让人确认可能是更合适的设计。只有在净收益增加、失败可观察、权限可限制且恢复可行时，才值得增加 AI 的动作权限。

澄川可以让参会人读取保持确定性自动化，让 AI 只对需求识别提出建议，并准备 CRM 更新预览，由销售确认后提交。即使未来允许 AI 在明确范围内创建内部任务，对外邮件仍应由人决定和发送。系统整体没有必要获得一个笼统的“半自动”标签。
...
选一个包含五个以上动作的任务，为每个动作写下：当前执行者、错误代价、可逆性、所需上下文、AI 只建议、准备内容还是可以执行，以及扩大权限前必须补的证据。至少保留一个确定性步骤和一个完全不使用 AI 的步骤。
...
[下一章：从工具愿望到可验证场景](./05-verifiable-scenario.md)
