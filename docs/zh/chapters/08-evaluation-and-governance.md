...
desc: "承接澄川沙盒方案，从一次被平均分掩盖的失败评审出发，建立结果、轨迹、环境与治理联合评估，并用故障演练判断系统是否可以进入小范围试点。"
aliases:
  - "Agent评估与治理"
  - "AI试点就绪"
tags:
...
首轮评估集有 24 个任务规格。这个数量只为了让教学案例能够覆盖当前风险，不是统计充分性的保证。系统越成熟、任务分布越复杂，样本和重复试验都应增加。
...
样本两边都要覆盖。若只测试“该写入时能否写入”，团队可能把系统调成凡事都提交；还要测试“不该写入时能否停下”。同样，既要测试资料冲突会阻塞，也要测试资料一致时不会无故拒绝。

每个任务至少保存样本编号、来源类型、输入与初始状态、期望环境结果、允许路径、禁止动作、风险级别、评分器和数据版本。来自真实工作的材料必须先获得授权并脱敏；虚构或合成样本要保留标签，不能混进真实事件统计。

模型参与的路径存在波动。澄川对高风险和历史回归任务运行多次试验，分别报告“至少一次成功”和“每次都成功”的含义，不把挑出一次好结果当作稳定性。具体重复次数由风险、成本和决策用途决定。
...
[NIST AI RMF Core](https://airc.nist.gov/airmf-resources/airmf/5-sec-core/)用 Govern、Map、Measure、Manage 组织 AI 风险管理，并把治理视为贯穿生命周期的活动。对澄川而言，这些词只有进入角色、配置和状态才有用。

Govern 对应谁可以决定。业务负责人批准试点范围，销售对提交内容负责，内容负责人维护资料，技术负责人管理适配器，数据与安全角色决定日志和授权。任何人都不能用“项目很紧急”替别人作专业批准。

Map 对应用途和影响。当前系统只帮助销售准备四个内部字段，并在人工确认后调用确定性写入；它不报价、不改商机阶段、不发外部消息。数据、动作和受影响对象的范围都写进风险登记。

Measure 对应本章的评估与监控。团队分别检查输出、轨迹、环境状态和人工决定，并保留失败样本。Manage 则把发现的问题变成权限调整、停止、回退、事件处理和版本决定。

治理控制不能只写在项目说明中。字段白名单存在于服务端；试点用户和商机存在于权限策略；人工批准存在于 `Approval` 对象；停止开关能撤销写入身份；日志保留策略由存储配置执行。评估要测试这些控制是否能被绕过。

## 用一次事件演练检查“出事以后”

决定是否开始试点前，项目组没有再制造一次真实事故，而是在沙盒运行事件演练。演练材料假设某次更新疑似写入了禁止字段，并且监控在五分钟后报警。
...
演练记录的是当前团队在模拟条件下的表现，不能证明真实事件会完全相同。它的价值是提前暴露名单、权限和工具缺口，让严重问题不必等到第一次事故才被发现。
...
只收集正常样本，会得到一个适合演示的系统。真实输入里的缺失、冲突、注入和权限变化都没有出现，发布后才成为用户发现的“新问题”。

只给最终回答打分，会漏掉危险的正确答案。系统也许从错误资料得出碰巧正确的结论，或者在生成正确文本后写错对象。轨迹与环境状态要分别检查。

让一个模型给所有内容评分，会把评审器的偏差变成统一标准。能确定计算的项目交给代码，专业判断交给领域人员，模型评分器用于扩展但要校准。

用平均分处理风险，会让大量简单样本淹没少量严重失败。评审应先检查必须停止的问题，再讨论总体质量和可接受误差。

把日志开得越多越安心，也是一种假象。过量记录会创造新的数据风险。轨迹应当足以回答谁、何时、使用什么版本、执行什么动作和环境发生什么；其余内容按故障用途另行授权。
