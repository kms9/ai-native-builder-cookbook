...
| 业务约束 | [第 6A 章](../chapters/06a-sop-to-agent-constraints.md) | [SOP 到智能体约束转化卡](./sop-to-agent-card.md) | 业务约束能否进入系统设计 |
...
> **澄川示例**
>
> E-007｜事实｜两名销售在六次观察中都先找产品资料，再更新 CRM｜屏幕观察记录｜中｜支持“资料查找属于现状流程”｜仍需确认更多销售是否相同。
>
...
> **澄川示例**
>
> 问题不是“销售不会写纪要”，而是会后需要在会议内容、产品资料和 CRM 之间反复核对，造成等待与返工。当前六次观察只能支持这一流程问题，不能支持“效率下降 30%”。第一轮只验证四个内部字段的建议是否减少查找和重复录入。
...
这张卡使用单独文件保存，避免把版本记录、异常表和系统映射压缩进本练习册。请打开并复制 [SOP 到智能体约束转化卡](./sop-to-agent-card.md)。
...
> **澄川示例**
>
> `CRMChangeSet` 表示一次待确认的 CRM 字段变更，不是模型的一段自由文本。它只能从草稿进入待审批、已批准、提交中、已完成、已阻塞或未知状态；未知状态不得创建新的业务意图。
...
> **澄川示例**
>
> `submit_crm_change` 只接受已经批准的 `CRMChangeSet`，字段白名单为四项，使用一次性业务幂等键。写入超时后先进入未知状态并查询原请求结果，不生成新键重试。价格和商机阶段即使出现在模型输出中，也会被工具拒绝。
